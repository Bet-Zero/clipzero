/**
 * Ad-hoc stats + CDN checks. Uses {@link selectVideoFromEventAsset} for
 * videoeventsasset (same rendition rules as the API), not a separate URL pick.
 */
import axios from "axios";
import { selectVideoFromEventAsset } from "../lib/nba";
...
async function main() {
  ...
  // Test 3: videoeventsasset — log full response and selected murl (shared helper)
  try {
    ...
    const selected = selectVideoFromEventAsset(r.data);
    console.log(
      "\n--- selectVideoFromEventAsset (rendition only; not CDN placeholder fix) ---",
    );
